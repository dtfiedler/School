...
class BTreeIndex {
 ...
 public:
   ...
   * BTreeIndex Destructor. 
     ...
     *
     */
    void assignKey(void *nodeptr, int pos, const void *key, bool isNL, bool clear);

    /**
     *
     *
     */
    void copyKeys(void *src, int start, int end, void *des, bool reverse = false);

    /**
     *
     *
     */
    int compareKey(const void *key1, const void *key2, bool isStr = true);


    /*
     * Search for the proper position of given key.
     * KT: keytype, NLT: non leaf node type, LT: leaf node type
     */
    template<typename KT, typename NLT, typename LT, int LS>
    PageId searchLeaf(const KT *key, PageId *pidpath);
     ...
     */
    template<typename KT, typename NLT, typename LT, int LS>
    KT splitLeaf(PageId target, const KT *key, const RecordId rid, PageId& newPid);
    ...
    KT splitNonLeaf(PageId target, const KT key, PageId& newPid, int& pos);
     ...
     */
    template<typename KT, typename NLT, typename LT, int NLS>
    int insertNonLeaf(const KT key, PageId *pidpath, int depth, PageId& attachPid);
     ...
     */
    template<typename KT, typename NLT, typename LT, int LS>
    bool insertLeaf(PageId target, const KT *key, const RecordId rid);
    ...
    void insertHelper(const KT *key, const RecordId rid);
   ...
   */
    template<typename KT, typename NLT, typename LT, int LS, int NLS>
    void scanLeaf(const void *lowValParm, const Operator lowOpParm, const void *highValParm, const Operator highOpParm);
   ...
   */
    template<typename KT, typename NLT, typename LT, int LS, int NLS>
    void scanHelper(const void *lowValParm, const Operator lowOpParm, const void *highValParm, const Operator highOpParm);
  ...
  /**
	...
	**/
    ...
    void printNode(void * node, bool isNL = false);
};

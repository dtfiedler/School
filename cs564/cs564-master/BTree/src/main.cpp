...
void sparseIntTests();
void sparseDoubleTests();
void sparseStringTests();
void intTests();
...
int main(int argc, char **argv)
{
  ...
  try
...

	{
    	...
    	sprintf(record1.s, "%06d string record", i);
    	record1.i = i;
    	record1.d = (double)i;
...
void createRelationSparse()
{
  ...
  int skip = 5;
  for(int i = 0; i < relationSize * skip; i+=skip )
	{
    sprintf(record1.s, "%06d string record", i);
    record1.i = i;
    record1.d = (double)i;
...
void createRelationRandom()
{
  ...
  while( i < relationSize )
  {
    ...
    val = intvec[pos];
    sprintf(record1.s, "%06d string record", val);
    record1.i = val;
    record1.d = val;
...
void intTests()
{
  ...
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	checkPassFail(intScan(&index,-1000,GT,6000,LTE), 5000)
	checkPassFail(intScan(&index,-1000,GT,6000,LT), 5000)
    /*
	checkPassFail(intScan(&index,49999,GTE,49999,LT), 0)
	checkPassFail(intScan(&index,49999,GT,49999,LT), 0)
	checkPassFail(intScan(&index,49999,GT,49999,LTE), 0)
	checkPassFail(intScan(&index,49999,GTE,49999,LTE), 1)
	checkPassFail(intScan(&index,-1000,GTE,6000,LT), 6000)
	checkPassFail(intScan(&index,-1000,GT,6000,LTE), 6001)
	checkPassFail(intScan(&index,-1000,GT,6000,LT), 6000)
    checkPassFail(intScan(&index, 100000,GTE,600000,LT),500000)
    */
}
...
void doubleTests()
{
  ...
  BTreeIndex index(relationName, doubleIndexName, bufMgr, offsetof(tuple,d), DOUBLE);
	...
	checkPassFail(doubleScan(&index,-1000,GTE,6000,LTE), 5000)
	checkPassFail(doubleScan(&index,-1000,GTE,6000,LT), 5000)
	checkPassFail(doubleScan(&index,-1000,GT,6000,LTE), 5000)
	checkPassFail(doubleScan(&index,-1000,GT,6000,LT), 5000)
    /*
	checkPassFail(doubleScan(&index,49999,GTE,49999,LT), 0)
	checkPassFail(doubleScan(&index,49999,GT,49999,LT), 0)
	checkPassFail(doubleScan(&index,49999,GT,49999,LTE), 0)
	checkPassFail(doubleScan(&index,49999,GTE,49999,LTE), 1)
	checkPassFail(doubleScan(&index,-1000,GTE,6000,LTE), 6001)
	checkPassFail(doubleScan(&index,-1000,GTE,6000,LT), 6000)
	checkPassFail(doubleScan(&index,-1000,GT,6000,LTE), 6001)
	checkPassFail(doubleScan(&index,-1000,GT,6000,LT), 6000)
    */
}
...
void stringTests()
{
  ...
  BTreeIndex index(relationName, stringIndexName, bufMgr, offsetof(tuple,s), STRING);
	...
	checkPassFail(stringScan(&index,-1000,GTE,6000,LTE), 5000)
	checkPassFail(stringScan(&index,-1000,GTE,6000,LT), 5000)
	checkPassFail(stringScan(&index,-1000,GT,6000,LTE), 5000)
	checkPassFail(stringScan(&index,-1000,GT,6000,LT), 5000)
    /*
	checkPassFail(stringScan(&index,30000,GTE,40000,LT), 10000)
	checkPassFail(stringScan(&index,49999,GTE,49999,LT), 0)
	checkPassFail(stringScan(&index,49999,GT,49999,LT), 0)
	checkPassFail(stringScan(&index,49999,GT,49999,LTE), 0)
	checkPassFail(stringScan(&index,49999,GTE,49999,LTE), 1)
	checkPassFail(stringScan(&index,-1000,GTE,6000,LTE), 6001)
	checkPassFail(stringScan(&index,-1000,GTE,6000,LT), 6000)
	checkPassFail(stringScan(&index,-1000,GT,6000,LTE), 6001)
	checkPassFail(stringScan(&index,-1000,GT,6000,LT), 6000)
    */
}
...
int stringScan(BTreeIndex * index, int lowVal, Operator lowOp, int highVal, Operator highOp)
{
  ...
  char lowValStr[100];
  sprintf(lowValStr,"%06d string record",lowVal);
  char highValStr[100];
  sprintf(highValStr,"%06d string record",highVal);
